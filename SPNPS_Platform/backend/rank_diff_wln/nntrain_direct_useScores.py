...
from rank_diff_wln.nn import linearND, linear
from rank_diff_wln.mol_graph_direct_useScores import atom_fdim as adim, bond_fdim as bdim, max_nb, smiles2graph
from rank_diff_wln.models import *
...
parser.add_option("-p", "--cand", dest="cand_path", default="../core_wln_global/model-fine-tune-3130/train3.cbond_detailed")
parser.add_option("-b", "--batch", dest="batch_size", default=4)
parser.add_option("-a", "--ncand", dest="cand_size", default=150)
parser.add_option("-c", "--ncore", dest="core_size", default=16)
parser.add_option("-m", "--save_dir", dest="save_path", default="model-fine-tune-ori")
parser.add_option("-w", "--hidden", dest="hidden_size", default=500)
parser.add_option("-d", "--depth", dest="depth", default=3)
parser.add_option("-n", "--max_norm", dest="max_norm", default=50.0)
parser.add_option("-l", "--model", dest="model_path", default="model-core16-500-3-max150-direct-useScores")
parser.add_option("--checkpoint", dest="checkpoint", default="2400000")
...
# fine-tune
if opts.checkpoint:
    restore_path = os.path.join(opts.model_path, 'model.ckpt-%s' % opts.checkpoint)
else:
...
fig_M = 20000
fig_batch = 2
fig_loss = np.zeros([fig_M])
fig_error = np.zeros([fig_M])
try:
    while not coord.should_stop():
        ...
        if it % fig_batch == 0 and it > 0:
            print("After %d, Training Error: %.4f, Param Norm: %.2f, Grad Norm: %.2f" % (it, sum_err / fig_batch, pnorm, sum_gnorm / fig_batch * batch_size))
            fig_loss[int(it / fig_batch - 1)] = sum_gnorm / fig_batch * batch_size
            fig_error[int(it / fig_batch - 1)] = sum_err / fig_batch
            sys.stdout.flush()
        ...
        if it % 40000 == 0 and it > 0:
            ...
            ax2 = ax1.twinx()
            lns1 = ax1.plot(np.arange(fig_M), fig_loss, label="Loss")
            lns2 = ax2.plot(np.arange(fig_M), fig_error, 'r', label="Error")
