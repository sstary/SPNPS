...
def test_stage3():
    predfile = open('./data/pred_result.txt', 'w')

    parser = OptionParser()
    parser.add_option("-t", "--pred", dest="pred_path", default='./rank_diff_wln/model-fine-tune-ori/pred.cbond_detailed_13800') # file containing predicted edits
    parser.add_option("-g", "--gold", dest="gold_path", default='./data/pred.txt.proc') # file containing true edits
    parser.add_option("-s", "--singleonly", dest="singleonly", default=False) # only compare single products
    parser.add_option("--bonds_as_doubles", dest="bonds_as_doubles", default=True) # bond types are doubles, not indices
    ...
    try:
        ...
        for line in fpred:
            ...
            predfile.write('\t'.join([str(x) for x in thisrow]) + '\n')
    finally:
        fpred.close()
        fgold.close()
        predfile.close()