...
parser = OptionParser()
parser.add_option("-t", "--train", dest="train_path", default="../data/train3_0.txt.proc")
parser.add_option("-m", "--save_dir", dest="save_path", default="model-fine-tune-ori3")

parser.add_option("-p", "--model", dest="model_path", default="model-300-3-direct")
# parser.add_option("-p", "--model", dest="model_path", default="model-fine-tune-ori1")
...
parser.add_option("-c", "--checkpoint", dest="checkpoint", default="ckpt-140000")
...
loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.to_float(flat_label), logits=flat_score)
...
def read_data(path, coord):
    ...
    # divide reactants into different buckets by atom numbers
...
try:
    while not coord.should_stop():
        ...
        if it % fig_batch == 0:
            print("Acc@10: %.4f, Acc@20: %.4f, Param Norm: %.2f, Grad Norm: %.2f" % (sum_acc / (fig_batch * batch_size), sum_err / (fig_batch * batch_size), pnorm, sum_gnorm / fig_batch) )
            fig_loss[int(it / fig_batch - 1)] = sum_gnorm / fig_batch
            fig_accuracy[int(it / fig_batch - 1)] = sum_acc / (fig_batch * batch_size)
            sys.stdout.flush()
        ...
        if it % 10000 == 0:
            ...
            lns2 = ax2.plot(np.arange(fig_M), fig_accuracy, 'r', label="Accuracy")
